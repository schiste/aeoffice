...
* **Tune (Reset):** Cash in accumulated Harmonics (incl. Harmonic Fragments carried back by the Hero) to recalculate `Resonance` for the next run at the cost of a hard reset (Crystal shutdown + all crew die); most buildings remain placed but become `detuned` (upgrades reset + one-time re-attunement payment), while `untuned` buildings keep upgrades, and you restart with no crew but big generation bonuses and new unlocks.
