...
import {
  compileSemanticMapDefinition as compileGameSemanticMapDefinition,
  firstFreeTile as findFirstFreeTile,
  tileSpawnPoint as createTileSpawnPoint,
  validateCompiledMap,
...
import type {
  ...
  PromptMapSpawnPoint,
  PromptMapValidation,
  SemanticFurniturePlacement,
  SemanticMapDefinition,
  SemanticWallPlacement,
} from "@aedventure/game-map"
...
export type {
  ...
  CompiledSemanticMap,
  CompiledTileLayer,
  MapDefinitionInterface,
  MapSpawnPoint,
  MapValidation,
  MapValidationCheck,
  PromptMapSpawnPoint,
  PromptMapValidation,
  PromptMapValidationCheck,
  SemanticFurniturePlacement,
  SemanticMapDefinition,
  SemanticObjectPlacement,
  SemanticWallPlacement,
  SemanticZoneDefinition,
} from "@aedventure/game-map"
...
export function compileSemanticMapDefinition(
  ...
  return compileGameSemanticMapDefinition(definition, catalog)
}
...
export function compilePresetMap(
  ...
  const spawnPoints = preset.spawnTiles.map((spawn) =>
    createTileSpawnPoint(spawn.id, spawn.tile, compiled.tileSize),
  )
  const validation = validateCompiledMap(compiled, spawnPoints)
...
function promptMapSpawnPoints(
  ...
  const defaultTile =
    findFirstFreeTile(compiled, preferredTiles) ?? { x: 1, y: 1 }
  const guestTile =
    findFirstFreeTile(
      compiled,
  ...
  return [
    createTileSpawnPoint("default", defaultTile, compiled.tileSize),
    createTileSpawnPoint("guest", guestTile, compiled.tileSize),
  ]
}
