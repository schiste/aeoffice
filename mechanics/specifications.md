...
* `H` (Harmonics meta multiplier): Long-term multiplier derived from Harmonics Level plus a small weighted contribution from total Harmonic Fragments. Used to scale re-attunement costs and many Base effects.
* `Resonance_next` (next-run Resonance): A per-run computed multiplier for the next run after Tuning (NGU-style). Exact factor breakdown TBD.
